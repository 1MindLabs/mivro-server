...
def update_message() -> Response:
    ...
    if not email or not old_message or not new_message:
        return (
            jsonify({"error": "Email, old message, and new message are required."}),
            400,
        )
    ...
    try:
        ...
        if savora_response.status_code != 200:
            runtime_error(
                "update_message",
                "Savora AI failed to process the message.",
                middleware=savora_response.json(),
                email=email,
            )
            return (
                jsonify({"error": "Savora AI failed to process the message."}),
                savora_response.status_code,
            )
