...
def update_profile() -> Response:
    ...
    try:
        ...
        if (dietary_preferences := request.json.get("dietary_preferences")) not in [
            None,
            "",
        ]:
            update_data["health_profile.dietary_preferences"] = dietary_preferences
        if (medical_conditions := request.json.get("medical_conditions")) not in [
            None,
            "",
        ]:
...
def favorite_product() -> Response:
    ...
    if not email or not product_name or not product_brand or not product_image:
        return (
            jsonify({"error": "Email, product name, brand, and image are required."}),
            400,
        )
    ...
    try:
        ...
        user_document.set(
            ...
            merge=True,
        )  # Merge the favorite product with the existing user document (if any)
...
def flag_product() -> Response:
    ...
    if not email or not product_name or not product_brand or not flag_reason:
        return (
            jsonify(
                {"error": "Email, product name, brand, and description are required."}
            ),
            400,
        )
    ...
    try:
        ...
        user_document.set(
            ...
            merge=True,
        )  # Merge the flagged product with the existing user document (if any)
