...
def signup() -> Response:
    ...
    try:
        ...
        )  # verify-email route not working :(
...
def signin() -> Response:
    ...
    try:
        ...
        if "message" in result:
            session["email"] = (
                email  # Store the user's email in the session if the login is successful
            )
...
def delete_account() -> Response:
    ...
    try:
        ...
        if "email" in session:
            session.pop(
                "email", None
            )  # Remove the user's email from the session if they are logged in
